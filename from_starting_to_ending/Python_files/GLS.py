...
econ_df_1 = pd.read_csv(
    os.path.join(file_path, "new_files/timeLag_2003.csv"), index_col=0
)
...
for ind_variables in independent_variables:
    ...
    for values_i in dependent_variables:
        ...
        name = (
            values_i
            + " and "
            + ind_variables.replace("_DISCLOSURE_SCORE_Robust", "")
            + ".txt"
        )
        # Save the model summary to a text file
        with open(os.path.join(file_path, "GLS_results/Robust", name), "w") as file:
