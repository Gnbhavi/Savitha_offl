...
df = pd.read_csv(
    "~/Python_github/savitha/Savitha_offl/from_starting_to_ending/panel_data_prediicting_2.csv",
    parse_dates=["Dates"],
    index_col="Dates",
)
...
df = df.drop(
    ["IS_ADVERTISING_EXPENSES", "IS_TOTAL_REVENUES", "ARDR_NI_TO_SE_RATIO"], axis=1
)

variable_to_be_filled = [
...
df_copy.to_csv(os.path.join(main_file_path, "independent_variables.csv"))
with open(
    os.path.join(main_file_path, "independent_variables_wasted_companies_dict.pkl"),
    "wb",
) as pickle_file:
